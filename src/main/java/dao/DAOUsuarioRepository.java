	...
	public void editarUsuario(ModelLogin modelLogin) {
		...
		try {
			if(existeUsuario(modelLogin.getLogin())) {
				String sql = "UPDATE model_login SET senha=?, nome=?, email=?, perfil=?, sexo=?, fotouser=?, extensaofotouser=?, datanascimento=?, rendamensal=? WHERE id= " + modelLogin.getId();
				PreparedStatement statement = connection.prepareStatement(sql);
				...
				if(modelLogin.getEndereco().getCep() != null && !modelLogin.getEndereco().getCep().isEmpty() && temEndereco(modelLogin.getId())) {
					sql = "UPDATE model_endereco SET cep=?, uf=?, cidade=?, rua=?, bairro=?, numero=? WHERE id_usuario=" + modelLogin.getId();
